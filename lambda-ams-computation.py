...
class AMSComputation:
    """AMS (Average Match Score) computation for challenge scoring"""
    ...
    def get_shortlist_data(self, target_date: str) -> List[Dict]:
        """Get shortlist data from S3 for the target date"""
        try:
            # Look for shortlist files in the curated bucket
            prefix = f"metrics/shortlists/date={target_date}/"
            ...
            logger.info(f"Found {len(shortlist_data)} shortlist records for {target_date}")
    ...
    def get_shortlist_data_from_athena(self, target_date: str) -> List[Dict]:
        """Get shortlist data from Athena query"""
        try:
            query = f"""
            SELECT 
                challenge_id,
            ...
            logger.error(f"Error getting shortlist data from Athena: {e}")
            return []
    ...
    def store_ams_metrics(self, metrics: Dict, target_date: str) -> str:
        """Store AMS metrics as Parquet file in S3"""
        try:
            ...
            # Upload to S3
            overall_key = f"ams/overall/date={target_date}/overall_{int(time.time())}.parquet"
            challenge_key = f"ams/challenges/date={target_date}/challenges_{int(time.time())}.parquet"
            ...
            logger.info(f"Stored AMS metrics to s3://{self.metrics_bucket}/{overall_key} and {challenge_key}")
            ...
            logger.error(f"Error storing AMS metrics: {e}")
            raise
...
def lambda_handler(event, context):
    """Main Lambda handler for AMS computation"""
    try:
        ...
        logger.info(f"Computing AMS metrics for {target_date}")
        
        # Initialize AMS computation
        ams_computation = AMSComputation(CURATED_BUCKET, METRICS_BUCKET, ATHENA_DATABASE, ATHENA_WORKGROUP)
        
        # Get shortlist data
        shortlist_data = ams_computation.get_shortlist_data_from_athena(target_date)
        ...
        # Compute AMS metrics
        metrics = ams_computation.compute_ams_metrics(shortlist_data)
        
        # Store metrics
        s3_keys = ams_computation.store_ams_metrics(metrics, target_date)
        ...
        return {
            'statusCode': 200,
            'body': json.dumps({
                'message': 'AMS computation completed successfully',
                'target_date': target_date,
                'total_challenges': metrics['total_challenges'],
                'total_shortlists': metrics['total_shortlists'],
                'avg_ams_challenge': metrics['avg_ams_challenge'],
        ...
        logger.error(f"Error in AMS computation: {e}")
        return {
            'statusCode': 500,
            'body': json.dumps({
                'error': str(e)
